...
class ThreeLayerConvNet(object):
    ...
    def __init__(
        ...
        pool_output_size = int(num_filters * (conv_output_size/2) * (conv_output_size/2))

        W1 = weight_scale * np.random.randn(num_filters, C, filter_size, filter_size)
        b1 = np.zeros(num_filters)


        W2 = weight_scale * np.random.randn(pool_output_size, hidden_dim)
    ...
    def loss(self, X, y=None):
        ...
        conv_relu_pool_out_flatten = conv_relu_pool_out.reshape(conv_relu_pool_out.shape[0], -1)
