...
def affine_backward(dout, cache):
    ...
    size = x.shape
    x_temp = x.reshape(size[0], -1)

    dx = np.dot(dout, w.T).reshape(size)
...
def layernorm_backward(dout, cache):
    ...
    var_batch += eps
    dx_norm = (dout.T * gamma).T                         
    dvar_batch = (-0.5) * dx_norm.T.dot(x_norm/var_batch).diagonal()
    dx_center = (2.0/N) * x_center * dvar_batch + dx_norm / np.sqrt(var_batch)
    dmean_batch = np.ones((D, N)).dot(-1.0 * dx_center).diagonal()
    
    dx = (dx_center + (1.0/N) * dmean_batch).T         
    dgamma = x_norm.dot(dout.T).diagonal()             
    dbeta = dout.dot(np.ones(D))                        
...
def conv_forward_naive(x, w, b, conv_param):
    ...
    col = im2col_books(x, FH, FW, stride, pad)
...
def conv_backward_naive(dout, cache):
    ...
    dx = col2im_books(dcol, x.shape, FH, FW, stride, pad)
...
def max_pool_forward_naive(x, pool_param):
    ...
    col = im2col_books(x, pool_height, pool_width, stride, 0)
...
def max_pool_backward_naive(dout, cache):
    ...
    dx = col2im_books(dcol, x.shape, pool_height, pool_width, stride, 0)
...
def spatial_batchnorm_forward(x, gamma, beta, bn_param):
    ...
    if mode == "train":
        ...
        running_mean = momentum * running_mean.reshape((1, C, 1, 1)) + (1 - momentum) * sample_mean
        running_var = momentum * running_var.reshape((1, C, 1, 1)) + (1 - momentum) * sample_var

        out = gamma.reshape((1, C, 1, 1)) * x_norm + beta.reshape((1, C, 1, 1))
        cache = (x, x_norm, gamma, norm, mean, sample_var, eps)
...
def spatial_batchnorm_backward(dout, cache):
    ...
    dsample_mean = np.sum(dx_hat * (-norm.reshape((1, C, 1, 1))), axis=(0, 2, 3))
    dsample_var  = np.sum((dx_hat * mean * (-1/2) * (sample_var.reshape((1, C, 1, 1)) + eps)**(-3/2)), axis=(0, 2, 3))
    dx = dx_hat * norm + dsample_mean.reshape((1, C, 1, 1)) * (1.0/N) + dsample_var.reshape((1, C, 1, 1)) * ((2.0/N )* (mean))
...
def spatial_groupnorm_forward(x, gamma, beta, G, gn_param):
    ...
    N, C, H, W = x.shape
    x_splite = x.reshape((N, G, C//G, H, W))
    sample_mean = np.mean(x_splite, axis=(2, 3, 4)).reshape(N, G, 1, 1, 1)
    sample_var  = np.var(x_splite, axis=(2, 3, 4)).reshape(N, G, 1, 1, 1)
    mean = (x_splite - sample_mean)
    norm = 1/np.sqrt(sample_var + eps)
    x_hat = mean * norm
    # x_hat = x_hat.T
    ...
    cache = (x, x_hat, gamma, norm, mean, sample_var, eps, G)
    out = out.reshape(N, C, H, W)
...
def spatial_groupnorm_backward(dout, cache):
    ...
    x, x_hat, gamma, norm, mean, sample_var, eps, G  = cache
    N, C, H, W = dout.shape
    group_size = H*W*C//G
    x_splite = x.reshape((N, G, C//G, H, W))
    ...
    dsample_mean = np.sum(dx_hat * (-norm), axis=(2,3,4))
    dx = dx_hat * norm + dsample_mean.reshape(N, G, 1, 1, 1) * (1.0/group_size) + dsample_var.reshape(N, G, 1, 1, 1) * ((2.0/group_size)* (mean))
